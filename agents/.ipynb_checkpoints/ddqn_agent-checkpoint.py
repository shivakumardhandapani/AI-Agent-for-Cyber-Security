...
class DDQNConfig:
    ...
    def __init__(self, 
                 ...
                 clip_norm=10.0):  # Gradient clipping parameter
        """
        ...
        :param tau: soft update rate for target network (Polyak averaging)
        :param clip_norm: maximum gradient norm for clipping
        """
        ...
        self.tau = tau  # Initialize tau correctly
        self.clip_norm = clip_norm  # Initialize clip_norm correctly
...
#         self.target_network_update_freq = target_network_update_freq
#         self.replay_memory_size = replay_memory_size
#         self.tau = tau  # Initialize tau
#         self.clip_norm = clip_norm  # Initialize clip_norm
    ...
    #     Initialize the configuration
        ...
        # self.clip_norm = 10.0  # Gradient clipping
        # self.epsilon_decay_good = 0.95
        # self.epsilon_threshold = 0.5


# Add these new classes to ddqn_agent.py

class TrainingLogger:
    """ Handles metric tracking and logging"""
    ...
    def log_episode(self, episode, reward, length, epsilon, loss):
...
class EvalLogger:
    ...
    def log_eval(self, rewards, success_rate):
        self.eval_rewards.append(np.mean(rewards))
...
class DDQNAgent:
    ...
    def __init__(self, env, config: DDQNConfig):
        ...
        self.epsilon_threshold = 0.5  # Performance threshold for faster decay
        self.epsilon_decay_good = 0.95  # Decay rate when performing well
        self.epsilon_decay_base = 0.995  # Base decay rate
        
        self.logger = TrainingLogger(log_dir)
        self.eval_logger = EvalLogger()
        self.epsilon_scheduler = AdaptiveEpsilonScheduler()

        self.tau = config.tau  # Soft update rate (0.005-0.01)
   ...
   def soft_update_target_network(self):
       ...
       new_weights = [
        self.config.tau * p_w + (1 - self.config.tau) * t_w  # Use config.tau
        for p_w, t_w in zip(policy_weights, target_weights)
    ]
    ...
    def train(self):
        ...
        while episode < self.config.num_episodes:
            ...
            while not done:
                ...
                # Store transition in replay buffer (with reward scaling)
                scaled_reward = reward * 0.1  # Scale rewards to [-0.1, 0.1]
                self.replay_buffer.push(state, action, scaled_reward, next_state, done)
                ...
                if len(self.replay_buffer) > self.config.batch_size:
                    ...
                    # Log loss history for debugging purposes
                    self.logger.loss_history.append(loss.numpy())
            ...
            # Adaptive epsilon decay based on performance threshold
            avg_reward = np.mean(self.episode_rewards[-100:]) if len(self.episode_rewards) >= 100 else np.mean(self.episode_rewards)
            if avg_reward > self.config.epsilon_threshold:
                self.epsilon *= self.config.epsilon_decay_good  # Faster decay for good performance
            else:
                self.epsilon *= self.config.epsilon_decay_base
            ...
            # Save avg reward over last 100 episodes for tracking progress
            avg_reward = np.mean(self.episode_rewards[-100:]) if len(self.episode_rewards) > 100 else np.mean(self.episode_rewards)
            self.avg_episode_rewards.append(avg_reward)
            
            # Update target network periodically or continuously (soft updates)
            ...
            if episode % self.config.train_log_frequency == 0:
                ...
                log_msg = f"Episode: {episode}, Reward: {episode_reward:.2f}, Avg Reward (100 ep): {avg_reward:.2f}, Epsilon: {self.epsilon:.4f}, Steps: {episode_steps}, Time: {elapsed_time:.2f}s"
        ...
        # Final save and evaluation after training completes
        self.save_model()
        final_eval_result = self.eval_model()
    ...
    #             if avg_reward > self.epsilon_threshold:
    #                 self.epsilon *= self.epsilon_decay_good
    #             else:
    #                 self.epsilon *= self.epsilon_decay_base
    #         else:
    #             self.epsilon *= self.epsilon_decay_base
